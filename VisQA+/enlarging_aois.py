...
def enlargeAOI(element_labels, im, ratio:float):
    df = pd.DataFrame(columns=['id', 'desc', 'file', 'x', 'y'])
    ...
    for row in element_labels.iterrows():
        ...
        expansion_poly = shrink_polygon(poly, ratio)
        #print(perimeter, area, expansion_poly)

        for i in range(np.shape(expansion_poly)[0]):
            df = df.append({'id': row[1][0], 'desc': row[1][1], 'file': row[1][2], 'x': expansion_poly[i][0], 'y': expansion_poly[i][1]}, ignore_index=True)
...
if __name__ == '__main__':
    ...
    cleaned_dir = os.path.join(os.path.dirname(args['element_labels_dir']), 'element_labels_enlarged')
    ...
    for path in glob(os.path.join(args['element_labels_dir'], '*')):
        ...
        with Image.open(img_path) as im:
            ...
            element_labels = enlargeAOI(element_labels, im, -0.05)
