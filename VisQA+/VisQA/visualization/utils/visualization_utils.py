...
def plot_img(im, DPI=100):
    ...
    fig = plt.figure(figsize=(width/DPI, height/DPI))  # , num=1, clear=True)
...
def plot_visualization(x, y, duration):
    ...
    for i in range(len(x)):
        if i == 0:
            li = plt.plot(x[i], y[i], '-o', color='red',
                          ms=25, alpha=0.99, zorder=10)
            plt.setp(li, 'markersize', 25)
        else:
            dur = duration[i] / 10 if duration[i] / 10 > 10 else 10
            dur = dur if dur < 15 else 15
            li = plt.plot(x[i], y[i], '-o', color='yellow',
                          ms=dur, alpha=0.6, zorder=5)
